...
class ScaledOverlayPlotter:
    ...
    def extract_variable(self, data, var_name):
        try:
            ...
            elif var_name.lower() == "recotime":
                selector = Select(verbosity=0)
                vector = Vector()
                trk_front = selector.select_surface(data['trkfit'], surface_name="TT_Front")
                trkfit_ent = ak.mask(data['trkfit']["trksegs"], trk_front)
                return np.array(ak.flatten(ak.drop_none(trkfit_ent['time']), axis=None))
    ...
    def plot_scaled_overlay(self, variable_name, output_file=None, 
        ...
        if variable_name == 'recotime':
            def_start, def_end = 475.0, 1650.0
            # Generate uniform distribution across the definition range
            cosmic_samples = np.random.uniform(def_start, def_end, large_sample_size)
        else:
            def_start, def_end = 95.0, 115.0
            # Default to Chebyshev distribution for momentum
            c1_mid = 0.219
            c2_mid = -0.108803
        ...
        # The remainder of your script seamlessly handles both arrays identically
        ...
        #ax.set_yscale('log')
        ax.set_ylim(ymin=1, ymax=60) # for time 300 45 if time cut
        ...
        ax.text(0.08, 0.93, "Preliminary\n(Summer 2026)", 
                fontsize=20, fontweight='normal', fontstyle='italic', ha='left', va='top', 
                transform=ax.transAxes, zorder=100)
        ...
        if logo_to_use:
            try:
                ...
                ax_logo = ax.inset_axes([0.65, 0.55, 0.2, 0.15]) # for time: [0.65, 0.55, 0.2, 0.15] # 60 if no RPC
                ax_logo.imshow(logo)
                ax_logo.axis('off')
        ...
        xlabel_map = {'recomom_ttfront': 'Reconstructed Momentum [MeV/c]', 'recomom_mc_ttfront': 'MC Momentum at Tracker Entrance [MeV/c]', 'recotime' : ' Track Time [ns]'}
        units_map = {'recomom_ttfront': '[MeV/c]', 'recomom_mc_ttfront': '[MeV/c]', 'recotime' : '[ns]'}
        ax.set_xlabel(xlabel_map.get(variable_name.lower(), variable_name))
        ax.set_ylabel(f'Events per {bin_width:.2f} {units_map.get(variable_name.lower(), "[units]")}')
